...
test('handleSelect is called when an season is selected', () => {
    const fakeHandleSelect = jest.fn(() => {
        return 'Fake handle Select';
    });

    render(<Show show={testShow} selectedSeason='none' handleSelect={() => fakeHandleSelect('fake')}/>)
    ...
    expect(fakeHandleSelect.mock.calls.length).tobe(1)
});
